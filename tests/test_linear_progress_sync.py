...
def test_current_marketplace_upgrades_previous_release_and_activates_prompt(tmp_path, monkeypatch):
    ...
    installed_prompt = (
        cache_root / "codex-session-logging/0.2.20/scripts/collective_feedback.py"
    )
    ...
    assert sorted(path.name for path in codex_versions.iterdir() if not path.name.startswith(".")) == ["0.2.20"]
    assert installed_prompt.read_bytes() == (
        ROOT / "plugins/codex-session-logging/scripts/collective_feedback.py"
    ).read_bytes()
    installed_skill = cache_root / "codex-session-logging/0.2.20/skills/search-coding-sessions/SKILL.md"
    assert installed_skill.read_bytes() == (
        ROOT / "plugins/codex-session-logging/skills/search-coding-sessions/SKILL.md"
    ).read_bytes()
    installed_manifest = json.loads((cache_root / "codex-session-logging/0.2.20/.codex-plugin/plugin.json").read_text())
