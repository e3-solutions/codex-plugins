...
def make_current_marketplace_archive(tmp_path: Path) -> tuple[Path, str]:
    repo_root = tmp_path / "current-payload" / "codex-plugins-main"
    ...
    marketplace["plugins"].append(
        {
            "name": "shared-employee-tools",
            "source": {"source": "local", "path": "./plugins/shared-employee-tools"},
            "policy": {"installation": "INSTALLED_BY_DEFAULT"},
        }
    )
    marketplace_path.write_text(json.dumps(marketplace, indent=2) + "\n", encoding="utf-8")
    archive = tmp_path / "current-marketplace.zip"
    with zipfile.ZipFile(archive, "w") as zip_file:
        for path in repo_root.rglob("*"):
            zip_file.write(path, path.relative_to(tmp_path / "current-payload"))
...
def test_setup_plan_is_global_by_default_and_does_not_install_repo_hook(tmp_path):
    ...
    assert "Linux systemd user timers" in notes
    assert "loginctl lingering" in notes
...
def test_setup_script_exists_and_exposes_dry_run():
    ...
    assert "Linux systemd user timers" in text
    assert "loginctl lingering" in text
...
def test_current_marketplace_upgrades_previous_release_and_activates_prompt(tmp_path, monkeypatch):
    update_plugin = load_update_plugin()
    ...
    first = update_plugin.run_update(
        current_plugin_root=old_linear,
        cache_parent=cache_parent,
        manifest_url=manifest.as_uri(),
        state_path=tmp_path / "update-state.json",
        force=True,
    ...
    new_linear = cache_parent / "0.3.22"
    ...
    installed_prompt = (
        cache_root / "codex-session-logging/0.2.22/scripts/collective_feedback.py"
    )
    assert first["updated"] is True
    assert second["updated"] is False
    assert second["skipped"] == "current"
    assert sorted(path.name for path in cache_parent.iterdir() if not path.name.startswith(".")) == ["0.3.22"]
    codex_versions = cache_root / "codex-session-logging"
    assert sorted(path.name for path in codex_versions.iterdir() if not path.name.startswith(".")) == ["0.2.22"]
    assert installed_prompt.read_bytes() == (
        ROOT / "plugins/codex-session-logging/scripts/collective_feedback.py"
    ).read_bytes()
    installed_skill = cache_root / "codex-session-logging/0.2.22/skills/search-coding-sessions/SKILL.md"
    assert installed_skill.read_bytes() == (
        ROOT / "plugins/codex-session-logging/skills/search-coding-sessions/SKILL.md"
    ).read_bytes()
    installed_manifest = json.loads((cache_root / "codex-session-logging/0.2.22/.codex-plugin/plugin.json").read_text())
    ...
    hook_env = {
        **os.environ,
        ...
        "E3_SESH_CONTEXT_ENABLED": "1",
        "E3_COLLECTIVE_FEEDBACK_HOOK_ENABLED": "1",
        "E3_SESH_START_SEARCH_ENABLED": "1",
    }
    hook_script = installed_prompt.parent / "session_start.py"
    startup = subprocess.run(
        [sys.executable, str(hook_script)],
        input=json.dumps({"cwd": str(repo), "session_id": "startup", "source": "startup"}),
        text=True,
        capture_output=True,
        check=False,
        env=hook_env,
    ...
    clear = subprocess.run(
        [sys.executable, str(hook_script)],
        input=json.dumps({"cwd": str(repo), "session_id": "clear", "source": "clear"}),
        text=True,
        capture_output=True,
        check=False,
        env=hook_env,
...
@pytest.mark.parametrize(
    ("runtime_commit", "registers_new_defaults_on_first_cycle"),
    (("d0e6f65", False), ("49aa765", True)),
)
def test_legacy_upgrade_keeps_presence_scheduler_decommissioned(
    tmp_path,
    monkeypatch,
    runtime_commit,
    registers_new_defaults_on_first_cycle,
):
    ...
    for name in ("linear_sync.py", "resident_updater.py", "update_plugin.py"):
        source = subprocess.run(
            ["git", "show", f"{runtime_commit}:{'plugins/linear-progress-sync/scripts'}/{name}"],
            cwd=ROOT,
            text=True,
            stdout=subprocess.PIPE,
            stderr=subprocess.PIPE,
    ...
    assert json.loads(first_cycle.stdout)["resident"]["version"] == "0.3.22"
    assert (resident_root / "runtime" / "current").resolve().name == "0.3.22"
    if registers_new_defaults_on_first_cycle:
        first_config = load_resident_updater().tomllib.loads(
            (codex_home / "config.toml").read_text(encoding="utf-8")
        )
        assert first_config["plugins"]["shared-employee-tools@coreedge-local"]["enabled"] is True
    ...
    assert result["updated"] is (not registers_new_defaults_on_first_cycle)
    assert result["resident"]["service"]["presence"] == {
        "changed": False,
        "decommissioned": True,
        "scheduled": False,
    }
    assert not (resident_root / "presence.sh").exists()
    assert not (home / "Library" / "LaunchAgents" / "com.coreedge.codex-session-presence.plist").exists()
...
def test_resident_marketplace_activation_is_atomic_and_deterministic(tmp_path):
    ...
    write_minimal_plugin(plugins_root, name="shared-employee-tools", version="0.1.0")
    (repo_root / ".agents" / "plugins").mkdir(parents=True)
    (repo_root / ".agents" / "plugins" / "marketplace.json").write_text(
        json.dumps(
            {
    ...
    assert sorted(path.name for path in (cache_root / "shared-employee-tools").iterdir() if not path.name.startswith(".")) == ["0.1.0"]
    ...
    assert '[plugins."linear-progress-sync@coreedge-local"]\nenabled = false' in config
    assert '[plugins."codex-session-logging@coreedge-local"]\nenabled = true' in config
    assert '[plugins."shared-employee-tools@coreedge-local"]\nenabled = true' in config
...
def test_resident_activation_restores_same_version_runtime_when_service_setup_fails(tmp_path, monkeypatch):
    ...
    def fail_service(*_args, **kwargs):
        assert kwargs["decommission_legacy_presence"] is False
    ...
    monkeypatch.setattr(
        ...
        lambda **_kwargs: decommission_calls.append(True),
    )
...
def test_real_marketplace_activates_in_isolated_codex_home_and_passes_doctor(tmp_path):
    ...
    assert activation["version"] == "0.3.22"
    assert health["healthy"] is True
    assert health["issues"] == []
    assert health["cache_versions"] == {
        "codex-session-logging": ["0.2.22"],
        "linear-progress-sync": ["0.3.22"],
    }
...
def test_resident_hook_repairs_matching_cache_and_runtime_corruption_from_managed_release(tmp_path):
    ...
    cache = codex_home / "plugins/cache/coreedge-local/linear-progress-sync/0.3.22"
...
def test_resident_runtime_and_launch_agent_are_idempotent(tmp_path, monkeypatch):
    ...
    assert not presence_plist.exists()
    assert not (tmp_path / "resident" / "presence.sh").exists()
    assert first["changed"] is True
    assert second["changed"] is False
    assert first["presence"]["decommissioned"] is True
    assert second["presence"]["decommissioned"] is True
    assert len(calls) == 2
...
def test_resident_runtime_and_systemd_user_units_are_idempotent(tmp_path):
    resident = load_resident_updater()
    plugin_root = write_minimal_plugin(
        tmp_path / "plugins",
        name="linear-progress-sync",
    ...
    updater_service = systemd_user_dir / "com.coreedge.codex-plugins-updater.service"
    ...
    assert not (systemd_user_dir / "com.coreedge.codex-session-presence.service").exists()
    assert not (systemd_user_dir / "com.coreedge.codex-session-presence.timer").exists()
...
def test_force_service_repair_restarts_active_systemd_timers(tmp_path):
    ...
    assert forced["presence"]["decommissioned"] is True
    assert calls == [
        ["systemctl", "--user", "enable", "com.coreedge.codex-plugins-updater.timer"],
        ["systemctl", "--user", "restart", "com.coreedge.codex-plugins-updater.timer"],
    ]
...
def test_systemd_future_retry_state_does_not_suppress_repair(tmp_path):
    ...
    Path(first["retry_state"]).write_text(
        json.dumps(
            {
                "label": resident.SERVICE_LABEL,
                "error": "future state",
    ...
    assert retried["presence"]["decommissioned"] is True
    assert calls == [["systemctl", "--user", "enable", "com.coreedge.codex-plugins-updater.timer"]]


def test_resident_systemd_user_timer_recovers_when_unit_is_inactive(tmp_path):
    resident = load_resident_updater()
    plugin_root = write_minimal_plugin(
        tmp_path / "plugins",
        name="linear-progress-sync",
    ...
    def runner(args, **_kwargs):
        calls.append(args)
        is_inactive_updater = (
            args[2:4] == ["is-active", "--quiet"]
            and args[-1] == "com.coreedge.codex-plugins-updater.timer"
        )
        return Completed(1 if is_inactive_updater else 0)
...
def test_linux_installer_keeps_hook_fallback_when_systemctl_is_unavailable(tmp_path):
    ...
    assert deferred["presence"]["decommissioned"] is True
    ...
    assert changed["presence"]["decommissioned"] is True
    ...
    assert forced["presence"]["decommissioned"] is True
    assert calls[-1] == ["systemctl", "--user", "enable", "com.coreedge.codex-plugins-updater.timer"]
    ...
    assert repaired["presence"]["decommissioned"] is True
...
def test_presence_launch_agent_is_decommissioned_idempotently(tmp_path, monkeypatch):
    resident = load_resident_updater()
    resident_root = tmp_path / "resident"
    launch_agents = tmp_path / "LaunchAgents"
    artifacts = [
        resident_root / "presence.sh",
        resident_root / "presence-service-active",
        resident.systemd_retry_state_path(
            resident_root=resident_root,
        ...
        launch_agents / f"{resident.PRESENCE_SERVICE_LABEL}.plist",
    ]
    ...
    monkeypatch.setattr(resident.os, "getuid", lambda: 501)
    first = resident.decommission_presence_publisher(
        codex_home=tmp_path / "codex",
        resident_root=resident_root,
        launch_agents_dir=launch_agents,
        platform="darwin",
        runner=lambda args, **_kwargs: calls.append(args) or Completed(),
    )
    second = resident.decommission_presence_publisher(
        codex_home=tmp_path / "codex",
        resident_root=resident_root,
        launch_agents_dir=launch_agents,
        platform="darwin",
    ...
    assert first == {"scheduled": False, "decommissioned": True, "changed": True}
    assert second == {"scheduled": False, "decommissioned": True, "changed": False}
    assert calls == [["launchctl", "bootout", "gui/501/com.coreedge.codex-session-presence"]]
    assert all(not path.exists() for path in artifacts)


def test_presence_systemd_units_are_decommissioned_idempotently(tmp_path):
    resident = load_resident_updater()
    resident_root = tmp_path / "resident"
    systemd_user_dir = tmp_path / "systemd" / "user"
    artifacts = [
        ...
        systemd_user_dir / f"{resident.PRESENCE_SERVICE_LABEL}.service",
        systemd_user_dir / f"{resident.PRESENCE_SERVICE_LABEL}.timer",
    ]
    for path in artifacts:
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text("legacy\n", encoding="utf-8")
    ...
    assert second == {"scheduled": False, "decommissioned": True, "changed": False}
    assert calls == [
        ["systemctl", "--user", "disable", "--now", "com.coreedge.codex-session-presence.timer"],
        ["systemctl", "--user", "daemon-reload"],
    ]
    assert all(not path.exists() for path in artifacts)


def test_presence_decommission_preserves_artifacts_when_launch_agent_stays_loaded(
    tmp_path,
    monkeypatch,
):
    resident = load_resident_updater()
    resident_root = tmp_path / "resident"
    launch_agents = tmp_path / "LaunchAgents"
    plist = launch_agents / f"{resident.PRESENCE_SERVICE_LABEL}.plist"
    ...
    def runner(args, **_kwargs):
        calls.append(args)
        return Completed(1 if args[1] == "bootout" else 0)

    monkeypatch.setattr(resident.os, "getuid", lambda: 501)
    result = resident.decommission_presence_publisher(
        codex_home=tmp_path / "codex",
        resident_root=resident_root,
        launch_agents_dir=launch_agents,
        platform="darwin",
...
def test_resident_doctor_reports_a_legacy_presence_scheduler_left_after_failed_cleanup(
    tmp_path,
):
    resident = load_resident_updater()
    codex_home = tmp_path / "codex"
    ...
    assert result["legacy_presence_artifacts"] == [str(plist)]
    assert "legacy one-minute session presence scheduler is still installed" in result["issues"]
...
@pytest.mark.parametrize(
    "configured_plugin",
    (
        'plugins."shared-employee-tools@coreedge-local".enabled = false\n',
        '[plugins]\n"shared-employee-tools@coreedge-local".enabled = false\n',
        '[ plugins . "shared-employee-tools@coreedge-local" ]\nenabled = false\n',
    ),
)
def test_default_plugin_registration_preserves_equivalent_disabled_toml(
    tmp_path,
    configured_plugin,
):
    resident = load_resident_updater()
    config = tmp_path / "config.toml"
    config.write_text(configured_plugin, encoding="utf-8")

    changed = resident.register_default_plugins(config, ["shared-employee-tools"])
    parsed = resident.tomllib.loads(config.read_text(encoding="utf-8"))

    assert changed is False
    assert parsed["plugins"]["shared-employee-tools@coreedge-local"]["enabled"] is False


def test_default_plugin_registration_ignores_header_text_in_multiline_string(tmp_path):
    resident = load_resident_updater()
    config = tmp_path / "config.toml"
    config.write_text(
        'notes = """\n[plugins."shared-employee-tools@coreedge-local"]\n'
    ...
    assert changed is True
    assert parsed["plugins"]["shared-employee-tools@coreedge-local"]["enabled"] is True


def test_default_plugin_registration_uses_codex_cli_without_tomllib(tmp_path, monkeypatch):
    resident = load_resident_updater()
    monkeypatch.setattr(resident, "tomllib", None)
    calls = []
                ...
                json.dumps(
                    {
                        "installed": [
                            {
                                "pluginId": "linear-progress-sync@coreedge-local",
                                "enabled": False,
                            }
                        ]
                    }
    ...
    changed = resident.register_default_plugins(
        tmp_path / "codex" / "config.toml",
        ["linear-progress-sync", "shared-employee-tools"],
        runner=runner,
    )
    ...
    assert [call[0] for call in calls] == [
        ...
        ["codex", "plugin", "add", "shared-employee-tools@coreedge-local", "--json"],
    ]
    assert all(call[1]["env"]["CODEX_HOME"] == str(tmp_path / "codex") for call in calls)


def test_resident_installer_degrades_safely_on_unsupported_platform(tmp_path):
    resident = load_resident_updater()
    plugin_root = write_minimal_plugin(
        tmp_path / "plugins",
        name="linear-progress-sync",
...
def test_doctor_does_not_require_legacy_presence_systemd_state(tmp_path):
    resident = load_resident_updater()
    codex_home = tmp_path / "codex"
    resident_root = tmp_path / "resident"
    systemd_user_dir = tmp_path / "systemd" / "user"
    ...
    assert health["healthy"] is True
    assert all("presence" not in key for key in health)
    assert all("presence" not in issue for issue in health["issues"])
...
def test_resident_doctor_ignores_retired_presence_state_and_queue(tmp_path, monkeypatch):
    resident = load_resident_updater()
    codex_home = tmp_path / "codex"
    resident_root = tmp_path / "resident"
    launch_agents = tmp_path / "LaunchAgents"
    ...
    assert all("presence" not in key for key in result)
    assert all("presence" not in issue for issue in result["issues"])
