...
def test_current_marketplace_upgrades_previous_release_and_activates_prompt(tmp_path, monkeypatch):
    ...
    new_linear = cache_parent / "0.3.27"
    ...
    installed_prompt = (
        cache_root / "codex-session-logging/0.2.28/scripts/collective_feedback.py"
    )
    assert first["updated"] is True
    assert second["updated"] is False
    assert second["skipped"] == "current"
    assert sorted(path.name for path in cache_parent.iterdir() if not path.name.startswith(".")) == ["0.3.27"]
    codex_versions = cache_root / "codex-session-logging"
    assert sorted(path.name for path in codex_versions.iterdir() if not path.name.startswith(".")) == ["0.2.28"]
    assert installed_prompt.read_bytes() == (
        ROOT / "plugins/codex-session-logging/scripts/collective_feedback.py"
    ).read_bytes()
    installed_skill = cache_root / "codex-session-logging/0.2.28/skills/search-coding-sessions/SKILL.md"
    assert installed_skill.read_bytes() == (
        ROOT / "plugins/codex-session-logging/skills/search-coding-sessions/SKILL.md"
    ).read_bytes()
    installed_manifest = json.loads((cache_root / "codex-session-logging/0.2.28/.codex-plugin/plugin.json").read_text())
...
def test_legacy_upgrade_keeps_presence_scheduler_decommissioned(tmp_path, monkeypatch):
    ...
    assert json.loads(first_cycle.stdout)["resident"]["version"] == "0.3.27"
    assert (resident_root / "runtime" / "current").resolve().name == "0.3.27"
...
def test_real_marketplace_activates_in_isolated_codex_home_and_passes_doctor(tmp_path):
    ...
    assert activation["version"] == "0.3.27"
    assert health["healthy"] is True
    assert health["issues"] == []
    assert health["cache_versions"] == {
        "codex-session-logging": ["0.2.28"],
        "linear-progress-sync": ["0.3.27"],
    }
...
def test_resident_hook_repairs_matching_cache_and_runtime_corruption_from_managed_release(tmp_path):
    ...
    cache = codex_home / "plugins/cache/coreedge-local/linear-progress-sync/0.3.27"
