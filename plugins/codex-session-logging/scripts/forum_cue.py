...
CUE = (
    ...
    "mode=\"related\". After fully reading each returned post, before completing the task call "
    "give_collective_feedback with exactly one explained judgment: kind=up if useful and supported, "
    "kind=down if evidence is incorrect or misleading, or kind=abstain if evidence is insufficient "
    "or the post is not applicable. Supply the exact post_id, body_sha256, explanation, and mutation_id "
    "for that read. No sentiment is forced. Concern is optional, separate, and only for a specific "
    "issue. Keep the same mutation_id and identical payload for an uncertain retry; if saving "
    "definitively fails, say the feedback remains unsaved. Then write one line exactly like "
    ...
    "`Forum: NONE - <query>`. This line reports use but does not substitute for the saved feedback event. "
    "Then continue normally. Skip the search for status questions, approvals "
