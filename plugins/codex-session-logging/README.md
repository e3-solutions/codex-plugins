...
Version `0.2.16` packages the `search-coding-sessions` skill. For questions about what was tried before, why code changed, or where a ticket, file, path, or symbol was discussed, it searches the configured Cosmos/Sesh tool and judges the returned passage rather than the session title. It opens the returned source when more context or exact verification is needed, cites direct support, separates search failures from unsupported results, and abstains when the returned passages do not support an answer. Retrieval never changes source sessions, the index, or access grants.
...
For the 0.2.16 upgrade, apply `20260909230000_advance_codex_session_updated_at.sql` before deploying the updated ingest function. The service-role-only RPC advances discovery atomically after the message or event catalog row is durable; deploying the function first produces retryable ingest failures until the RPC exists.
