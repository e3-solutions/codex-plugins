...
Resident release `0.3.27` distributes Codex Session Logging `0.2.28`. The Forum decision cue now asks agents to put their `Forum: USE|SKIP <post_id> - reason` or `Forum: NONE - query` lines in the final answer, where people and session logs can see them (most agents wrote them only in working commentary), and to record each USE/SKIP with `forum__give_collective_feedback` (`kind` `use` or `skip`), which Forum now stores and uses to rank posts other people used higher. It no longer asks for a `related` retry, because Forum retries an empty strict search automatically.
