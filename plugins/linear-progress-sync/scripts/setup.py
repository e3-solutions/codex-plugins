...
def print_summary(result: dict) -> None:
    ...
    if result["ok"]:
        ...
        print("2. Set E3_MCP_ACCESS_CODE in the environment that launches Codex.")
        print("3. If Codex asks to review hooks, trust the Linear Progress Sync and Codex Session Logging hooks once.")
        print("4. Start a new Codex thread so hooks, skills, and MCP servers reload.")
        print("5. On first use, Codex will list Linear users/projects and ask what to save.")
        print("")
        print("Notes:")
        ...
        print("- E3 MCP uses E3_MCP_ACCESS_CODE as its bearer token; access codes are never stored in this repository.")
        print("- Repos can opt out with linear_start.py configure-repo --disable-linear-sync.")
        print("- Codex Session Logging is installed during setup and updated with default marketplace plugins on SessionStart.")
