...
Existing installations activate `0.3.15` during one ordinary resident check and self-heal without rerunning setup. This release updates Codex Session Logging `0.2.16` with an evidence-first Sesh retrieval skill and a post-child ingestion watermark, registers E3 Cosmos during fresh setup, and adds a read-only teammate readiness check. It preserves the existing access model, repository synchronization guard, historical-backfill protections, Collective guidance, complete hook-triggered parent and subagent rollout capture, and the monotonic usage RPC migration. Do not ask teammates to reinstall, deliberately restart Codex, or create a renewal thread for normal updates; a normal future task picks up the new skill.
