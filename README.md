...
To let Tweed own the complete workflow globally, run:

```bash
python3 plugins/linear-progress-sync/scripts/linear_start.py workflow-owner --set tweed
```

This stores `"workflow_manager": "tweed"` in `~/.codex/linear-sync/repos.json` and makes Linear Progress Sync inert. Clear it with `linear_start.py workflow-owner --clear` to restore the native workflow.
