...
Existing installations download and activate `0.3.11` during one ordinary resident check. Codex Session Logging `0.2.12` keeps exact parent and subagent rollout bytes, canonical message history, tool and reasoning records, environment snapshots, every cumulative usage observation, deterministic crash recovery, and hook-triggered auto-drain. It removes the disabled historical importer, the retired one-minute presence publisher, packaged test scaffolding, unreachable backfill-write code, and repository-based session admission filters; every Codex task can now deliver records, using `codex://unscoped` when no Git origin exists. Older backfill payloads are still acknowledged without writes, old presence schedulers are still removed during upgrade, and installations self-heal without rerunning setup. Fresh setup installs and schedules the current plugins immediately.
